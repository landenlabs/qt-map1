...
static QString jsonString(const QJsonObject &obj, const QString &key, const QString &def = QString()) {
...
void GridManager::setApiKey(const QString &key) {
    m_apiKey = key;
}
...
QVariantList GridManager::grids() const {
    return m_gridsVariant;
}

// ─── JSON parsing ─────────────────────────────────────────────────────────────

QVector<GridManager::GridDef> GridManager::parseJson(const QByteArray &data, const QString &src) {
    QVector<GridDef> result;

    QJsonParseError err;
    const QJsonDocument doc = QJsonDocument::fromJson(data, &err);
    if (err.error != QJsonParseError::NoError) {
        qWarning("GridManager: JSON parse error in '%s': %s", qPrintable(src), qPrintable(err.errorString()));
        return result;
    }
    if (!doc.isObject()) {
    ...
    for (const QJsonValue &v : items) {
        ...
        if (gd.urlData.isEmpty()) {
            qWarning("GridManager: skipping '%s' — no urldata", qPrintable(gd.name));
            continue;
        }
    ...
    qInfo("GridManager: loaded %d grid(s) from '%s'", (int) result.size(), qPrintable(src));
    return result;
}
...
QString GridManager::substituteKP(const QString &tmpl, const QString &product) const {
    return QString(tmpl).replace("{k}", m_apiKey).replace("{p}", product);
}

// ─── Grid enable / disable ────────────────────────────────────────────────────

void GridManager::enableGrid(int index) {
    if (index < 0 || index >= (int) m_grids.size())
        return;
