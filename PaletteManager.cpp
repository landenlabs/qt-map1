...
void PaletteManager::loadFromBytes(const QByteArray &data, const QString &src) {
    ...
    if (err.error != QJsonParseError::NoError) {
        qWarning("PaletteManager: JSON parse error in '%s': %s", qPrintable(src), qPrintable(err.errorString()));
        return;
    }

    const QJsonObject palettes = doc.object().value(QStringLiteral("palettes")).toObject();
    ...
    for (auto it = palettes.constBegin(); it != palettes.constEnd(); ++it) {
        ...
        if (steps.isEmpty()) {
            qWarning("PaletteManager: palette '%s' has no steps — skipped", qPrintable(it.key()));
            continue;
        }
...
QImage PaletteManager::buildImage(const QJsonArray &steps, int resolution) {
    ...
    for (int i = 0; i < resolution; ++i) {
        ...
        auto clamp01 = [](float v) -> uchar { return static_cast<uchar>(std::clamp(v, 0.0f, 1.0f) * 255.0f + 0.5f); };
